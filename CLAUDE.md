...
Pulse Agent — AI-powered OpenShift/Kubernetes SRE and Security agent built on Claude. Connects to live clusters via the K8s API and uses Claude Opus for diagnostics, incident triage, and automated remediation. v2.29.0, Protocol v2, 107 native tools plus whatever connected MCP servers add, 7 skills (built-in: sre, security, view_designer, capacity_planner, plan-builder, postmortem, slo-management), 27 scanners (11 reactive + 5 audit + 1 SLO burn + 1 security posture + 5 predictive trend + 4 liveness: stuck, hot_loop, control_plane, degraded), 3338 tests, 83 PromQL recipes, 16 eval suites, 192 scenarios. 54 modules (k8s_tools/11, monitor/18, api/21, plus decorators.py, tool_predictor.py, inbox.py, inbox_generators.py, observability.py) — no file over 910 lines. Python 3.11+, Mypy clean (0 errors), ruff clean. Migration version 036 (runtime artifacts). Runtime-written documents are durable (`artifact_store.py`, `runtime_artifacts` + `runtime_artifact_versions`): skills, plan templates, scaffolded eval scenarios/fixtures and version history were written only to the container overlay filesystem and erased by every restart — write-through on change, replayed onto disk at boot before loaders scan. Plans can be created (`POST /plan-templates`) and are versioned like skills, with prior revisions at `GET /plan-templates/{type}/versions`. Durable plan execution on Temporal (`temporal/`, docs/TEMPORAL.md), inert unless `PULSE_AGENT_TEMPORAL_HOST` is set and provisioned by the operator's `spec.temporal`: one interpreter workflow runs any plan definition — including plans created in the UI — so a new workflow is data, not a deploy. Runs survive pod restarts and `approval_required` phases genuinely wait for a human signal (24h default) instead of being skipped as `needs_escalation`. Activities reuse `PlanRuntime._execute_phase`, so contract behaviour is identical on both paths; the monitor's automatic path still uses the in-process engine. `PULSE_AGENT_DURABLE_AUTOFIX` (off by default, needs Temporal) routes the monitor's post-approval fix sequence — snapshot, apply, verify, settle, recheck — through `IncidentWorkflow`, which is the part a pod restart loses today; any problem falls back to inline rather than dropping an approved fix. Workflow changes are a versioned operation — `workflow.patched` guards command-sequence changes, the worker stamps `build_id`. Verification contracts on the five most-used interactive write tools (`tool_contracts.py`): precondition read under the caller's token, pre-write snapshot, tool-specific postcondition probe on the monitor's verification pipeline (grace window for rollouts), evidence in fix history; offline replay suspends contracts. Harness deny policy (`policy.py`) enforced in the tool-execution path *after* confirmation: `delete_pod` is refused in `PULSE_AGENT_PROTECTED_NAMESPACES` (default production + platform; `restart_deployment` stays open as the reviewable path) and `drain_node`/`cordon_node` require `PULSE_AGENT_ALLOW_NODE_OPS` — safety posture set by configuration, not by which model is configured. `PULSE_AGENT_TRUST_LEVEL` (operator name) now accepted as alias of `PULSE_AGENT_MAX_TRUST_LEVEL`. Auto-routing orchestrator with typo auto-correction (~130 K8s misspellings), hard pre-route regex rules, and pre-route handoff in skill classifier. Centralized Pydantic config (no raw os.environ). Generative views: tools return 25 component specs (including action_button, confidence_badge, resolution_tracker, blast_radius, status_pipeline) for rich UI rendering, user-scoped custom dashboards with share/clone, action execution endpoint. Intelligence bridge: `normalize_component_spec()` / `normalize_layout()` in `quality_engine.py` fix field aliases (label→name, values→data, props flattening) before validation or persistence; `validate_layout_kinds()` gates REST and WS save paths; frontend `normalizeAgentProps.ts` provides matching client-side normalization. Agent view lifecycle: 3 view types (incident/plan/assessment) with status state machines, multi-user claims, finding dedup, recurrence handling, assessment→incident escalation. ViewEventBus for real-time broadcast. Multi-datasource live tables with K8s watches + PromQL/log enrichment + sparkline charts. Tool usage tracking: full audit log with chain intelligence. Adaptive tool selection: TF-IDF + LLM fallback + chain expansion. ORCA multi-signal skill selector (6-channel fusion, 5 active by default, parallel multi-skill execution max 2 with Sonnet synthesis, exclusive skills bypass secondary selection, bidirectional conflicts_with), phased plan execution, dependency graph (17 resource types, 10 relationships, 5 topology perspectives with metrics enrichment), auto-postmortems, SLO registry. ALWAYS_INCLUDE trimmed from 12 to 5. Episodes: one event with a cause, not N findings that are wrong — `episodes`/`episode_symptoms` tables, causal layers (infrastructure → platform → workload → signal) where a finding may only be explained by one strictly beneath it, symptoms collapsed out of the inbox with a count, and detachment recorded as training signal. Only infrastructure/platform findings with `findingType: "current"` may head an episode; forecasts and posture findings never can. Firing alerts are layered by *what the alert is about* (`monitor/alert_layers.py`) rather than by the fact that they arrived as alerts — an unclassified alert stays at the signal layer, where it can be a symptom but never a cause. Findings may carry `layer`, `posture` and `startedAt`; `startedAt` is the condition's own onset (Prometheus `activeAt`) and episodes correlate on it in preference to the monitor's in-memory first-sight, which is lost on every restart. A symptom belongs to exactly one episode, and a finding something deeper already explains does not head its own. A verification verdict has a time horizon: a finding returning with the same correlation key within `PULSE_AGENT_RECURRENCE_WINDOW` (default 1800s) of a verified action downgrades that action to `verified_then_recurred` / outcome `recurred` (`monitor/recurrence.py`), demotes the promoted trajectory (`learning_candidates` status `recurred`), walks back the confirmed memory incident (outcome `regressed`, runbook `failure_count`), and files an inbox review item for any skill deepened on that case; the auto-fix attempt cap also counts completed actions from the DB so a pod restart cannot reset it. The monitor scan loop starts in the app lifespan, not on WebSocket connect. Scanners that swallow an exception must call `report_failure(e)` — enforced by the `no-silent-scanner-failure` discipline rule. Release gate: 97.5% (release suite avg, 19/19 pass).

**UI Repository:** `/Users/alimobrem/apps/Pulse/pulse-ui` — React/TypeScript frontend (Zustand stores, incident views, admin dashboard).
...
- `inbox_generators.py` — 15 proactive task generators (cert expiry, trend prediction, degraded operators, upgrades, SLO burn, capacity, stale findings, privileged workloads, RBAC drift, network policy gaps, route certs, endpoint gaps, readiness regressions, skill gaps, skill low performers)
...
- `snapshot.py` — restorable copy of a resource captured immediately before an auto-fix writes to it. `before_state` was a human-readable sentence you cannot restore from, and rollback was limited to the three restart tools and gated on a status that never occurs on a trust-2 cluster. Strips server-owned fields (resourceVersion, uid, managedFields) that would make a replay conflict or lie. Migration 034
...
- `skill_lifecycle.py` — a verified trajectory deepens the skill that exists instead of cloning it. Learning keys on the finding category (`incident_type` stamped in scaffolded frontmatter): the first verified case creates the skill, later ones refine it in place — distilled case appended (capped at 5), keywords/required tools merged, version bumped. Refinement re-opens the review gate (a person approved v3, not v4) and says so via an inbox item rather than silently unrouting. Skill gaps and >30%-override skills surface as inbox proposals (`gen_skill_gaps`, `gen_skill_low_performers`) with auto-resolve, replacing the flywheel's log-only calls. `quarantined` frontmatter flag + POST /admin/skills/{name}/quarantine|unquarantine pulls a skill from both ORCA selection and trigger-pattern pre-route (pre-route now also honors the review gate) while keeping it loadable by name
- `trajectory.py` — verified-trajectory learning gate, persisted in `learning_candidates` (migration 033). It was an in-memory dict, which meant a pod restart wiped the pending set — and since promotion happens on a later scan than the investigation, it could only ever learn within one pod lifetime. Resolved rows are kept, so `stats()` reports the whole history rather than this process's counters. Investigations record a `LearningCandidate` instead of scaffolding immediately; `monitor/verification_pipeline.py` promotes it to a skill/plan/eval only once the finding is confirmed resolved, discards it if the fix did not hold, and the daily flywheel expires candidates whose verification never arrived. Learning is gated on outcome, not on self-assessed confidence
...
| `PULSE_AGENT_RECURRENCE_WINDOW` | How long a "verified" verdict stays answerable (seconds, 0=off) | `1800` |
...
| `PULSE_AGENT_PROTECTED_NAMESPACES` | Namespaces where raw pod deletion is denied (comma list, `*` wildcards; empty disables) | `production,openshift-*,kube-system` |
| `PULSE_AGENT_ALLOW_NODE_OPS` | Break-glass: permit `drain_node`/`cordon_node` | `false` |
