...
1. **Current state** — `list_resources(resource="nodes")` + `get_node_metrics()` for utilization per node
2. **Headroom** — calculate (allocatable - requested) / allocatable per node
3. **Hotspots** — identify nodes above threshold; use `list_resources(resource="resourcequotas", namespace=ns)` for namespaces overcommitting
